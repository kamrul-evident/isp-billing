...
export default function CustomerPaymentCreatePage() {
  ...
  const fetchCustomerData = useCallback(async () => {
    try {
      ...
      // Pre-fill amount with package price if available
      if (customerData.package?.price) {
        setFormData(prev => ({ ...prev, amount: customerData.package.price || '' }));
      }
    } catch (error: unknown) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/customers/${uid}/payments`);
    } catch (error: unknown) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div className="flex items-center">
              <Link href={`/customers/${uid}/payments`} className="text-gray-500 hover:text-gray-700 mr-4">
                ...
                {customer.package ? (
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm font-medium text-gray-900">{customer.package.name}</p>
                    </div>
                    <div>
